...
- 新装应用自动进入「未分类」兜底；分类页可批量打标签，应用很多时支持**搜索 + A-Z 字母索引**（中文按拼音）快速定位。
...
### 应用详情
- 今日 / 本周 / 本月 / 本年 / 永久时长，平滑趋势波形图 + 24 小时时间轴。
...
- **趋势波形图**：平滑曲线 + 渐变面积，只显示有使用的小时 / 日期，空数据居中提示。
- **主题**：跟随系统 / 浅色 / 深色，12 种整套界面色调（Material 3，背景、卡片、导航、弹窗整体变色）。
